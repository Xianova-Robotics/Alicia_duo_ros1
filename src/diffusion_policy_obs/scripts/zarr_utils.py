# 该文件定义了一个 ZarrDataCreator 类，用于创建和管理 Zarr 数据集。
...
class ZarrDataCreator:
    ...
    def create_zarr_store(self):
        ...
        """
        self.data_group.create_dataset('action', shape=(0, 7), dtype='f8', chunks=(100, 7))
        ...
        self.data_group.create_dataset('robot_joint', shape=(0, 7), dtype='f8', chunks=(100, 7))
    ...
    def delete_data(self):
        """
        删除 episode_ends 的最后一条数据，并删除 data_group 中对应范围的数据。
        """
        # 获取 episode_ends 数据集
        episode_ends_dataset = self.meta_group['episode_ends']
        if episode_ends_dataset.shape[0] == 0:  # 如果数据集为空，直接返回
            return

        # 获取倒数第二条 episode_end 数据
        last_episode_end = episode_ends_dataset[-2]

        # 删除 episode_ends 的最后一条数据
        episode_ends_data = episode_ends_dataset[:]  # 读取所有数据
        episode_ends_data = episode_ends_data[:-1]  # 删除最后一条数据
        episode_ends_dataset.resize(episode_ends_data.shape)  # 调整数据集大小
        episode_ends_dataset[:] = episode_ends_data  # 重新写入数据

        # 删除 data_group 中对应范围的数据
        for dataset_name in self.data_group.keys():
            dataset = self.data_group[dataset_name]
            if dataset.shape[0] > last_episode_end:  # 确保数据集有足够的数据
                data = dataset[:]  # 读取所有数据
                data = data[:last_episode_end]  # 删除 last_episode_end 之后的数据
                dataset.resize(data.shape)  # 调整数据集大小
                dataset[:] = data  # 重新写入数据
                
        return last_episode_end
