...
class DataSubscriber:
    def __init__(self, fps=30, camera_index=[0, 1], output_videos='./real_pusht/videos',output_obs='./real_pusht/replay_buffer.zarr'):
        """
        初始化视频录制器和数据订阅器。

        参数:
            fps (int): 视频帧率（默认 30）。
            camera_index (list): 摄像头的索引列表（默认 [0, 1]）。
            output_videos (str): 保存视频的根目录（默认 './real_pusht/videos'）。
            output_obs (str): 保存观测数据的路径（默认 './real_pusht/replay_buffer.zarr'）。
        """
        self.fps = fps
        self.camera_index = camera_index  # 摄像头索引列表
        self.output_video_root_dir = output_videos
        self.is_recording = False  # 是否正在录制视频
        self.is_saving = False  # 是否正在保存数据
        self.caps = {}  # 摄像头对象字典
        self.ffmpeg_processes = {}  # ffmpeg 进程字典
    ...
    def start_recording(self):
        ...
        try:
            ...
            for idx in self.camera_index:
                self.caps[idx] = cv2.VideoCapture(idx)  # 初始化摄像头
                self.caps[idx].set(cv2.CAP_PROP_FRAME_WIDTH, 640)
                self.caps[idx].set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            ...
            self.output_video_paths = {
                idx: os.path.join(output_folder, f'{idx}.mp4') for idx in self.camera_index
            }
            ...
            # 使用 ffmpeg 开始视频录制
            for idx in self.camera_index:
                self.ffmpeg_processes[idx] = ffmpeg.input('pipe:0', format='rawvideo', pix_fmt='bgr24',
                                                         s=f'{width}x{height}', r=self.fps) \
                                                     .output(self.output_video_paths[idx], vcodec='libx264', pix_fmt='yuv420p') \
                                                     .overwrite_output() \
            ...
            while self.is_recording:
                for idx in self.camera_index:
                    ...
                    # 将帧写入 ffmpeg 的标准输入
                    self.ffmpeg_processes[idx].stdin.write(frame.tobytes())
            ...
            # 释放资源
            for idx in self.camera_index:
                if idx in self.caps and self.caps[idx]:  # 检查摄像头是否存在
                    self.caps[idx].release()
                if idx in self.ffmpeg_processes and self.ffmpeg_processes[idx]:  # 检查 ffmpeg 进程是否存在
                    self.ffmpeg_processes[idx].stdin.close()
                    self.ffmpeg_processes[idx].wait()
...
def main():
    # 初始化 ROS 节点
    rospy.init_node('obs_data_node', anonymous=True)
