#!/usr/bin/env python
# -*- coding: utf-8 -*-
# 当在验证已经训练过的模型阶段，该脚本用于捕获多个摄像头的图像数据和机器人关节数据，并提供一个服务，用于处理这些数据。
...
class DataSaverNode:
    def __init__(self, camera_index=[0, 2]):
        """
        初始化 DataSaverNode。
        ...
        self.req = {
            'obs': {
                ...
                'camera_2': np.zeros((3, 240, 320), dtype=np.uint8),  # 默认图像数据
        ...
        while not rospy.is_shutdown():
            for idx in self.camera_index:
                ...
                with self.lock:
                    ...
                    print(f"Updated camera_{idx} data.", resized_frame.shape)
    ...
    def list_cameras(max_tests=5):
        for i in range(max_tests):
            ...
            if cap.isOpened():
                ...
                print(f"摄像头 {i} 不可用。")
    data_saver = DataSaverNode(camera_index=[0, 2])  # 使用摄像头索引 [0, 1]
    rospy.spin()
