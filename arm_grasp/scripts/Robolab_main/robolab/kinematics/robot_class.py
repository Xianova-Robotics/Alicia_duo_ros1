...
class RobotModel:
    ...
    def _resolve_mesh_path(self, mesh_path):
        if mesh_path.startswith("package://"):
            ...
            tokens = mesh_path.replace("package://", "").split("/")
            pkg_name = tokens[0]
            rel_path = "/".join(tokens[1:])
    ...
    def get_link_mesh_map(self):
        ...
        if self.robot_model_path.endswith('.urdf'):
            ...
            for link, link_info in self.robot_model.link_map.items():
                if link_info.visual is None:
                    print(f"[WARN] Link '{link}' has no visual")
                    continue
                if link_info.visual.geometry is None:
                    print(f"[WARN] Link '{link}' has no geometry in visual")
                    continue
                if not hasattr(link_info.visual.geometry, 'filename'):
                    print(f"[WARN] Link '{link}' visual geometry has no filename")
                    continue

                # raw_path = link.visual.geometry.filename
                # mesh_path = self._resolve_mesh_path(raw_path)
                # link_mesh_map[link_name] = mesh_path
                link_mesh_map[link] = {}
                mesh_path = link_info.visual.geometry.filename
                mesh_name = link_info.visual.geometry.filename.split("/")[-1].split(".")[0]
    ...
    def get_trans_dict(self, joint_value: List, base_trans: Union[None, torch.Tensor] = None) -> dict:
        ...
        original_base_trans = ret[self.base_link].get_matrix().to(self.device)
