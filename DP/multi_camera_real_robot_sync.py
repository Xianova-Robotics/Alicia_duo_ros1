# This script is used to control the real robot with multiple cameras using the Diffusion Policy.
# 同步：数据的获取与模型推理是同步的，运行速度会略微变慢，但是操控机械臂的准确性会提升。
...
class PolicyWrapper:
    def __init__(self, policy, n_obs_steps=2, n_action_steps=8):
        ...
        self.obs_history = deque(maxlen=self.n_obs_steps)  # 创建一个双端队列用于存储观察历史
        self.index = 0
        self.policy = policy  # 保存策略对象
        self.start_of_episode = True  # 标记是否是新的一轮（episode）的开始
        self.act_sequence = []  # 存储生成的动作序列

    def reset(self):
        # 重置方法，用于重置策略和队列
        self.obs_history.clear()  # 清空观察历史
    ...
    def step(self, obs):
        ...
        # obs: 当前的观察数据
        if self.start_of_episode:  # 如果是新的一轮的开始
            # 将观察数据添加到观察历史中
            self.obs_history.append(obs)

        # 如果观察历史的长度达到 n_obs_steps，生成动作序列
        if len(self.obs_history) == self.n_obs_steps:
            self.start_of_episode = False  
            obs_sequence = list(self.obs_history)  # 将观察历史转换为列表
            print("推理输入:", [obs['robot_joint'] for obs in obs_sequence])
            # 使用策略生成动作序列
            self.act_sequence = self.policy.step(obs_sequence)
            self.index += 1
            print("生成动作序列:", self.act_sequence, self.index)
...
class PolicyServer:
    ...
    def publish_action(self, latest_action):
        ...
        print("发送推理控制action:", action)
        print("发送推理控制gripper_angle:", float(latest_action[-1]))
        # 发布消息
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            if response.success:
                ...
                camera_2_flat = np.frombuffer(obs_data.camera_2, dtype=np.uint8)
